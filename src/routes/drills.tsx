...
export const Route = createFileRoute("/drills")({
  ...
  component: PublicDrillsRoute,
});

function PublicDrillsRoute() {
  const { t } = useT();

  return (
    <PublicShell title={t("public.publicDrills.title")}>
      <EmptyState
        icon={<LuTarget className="h-5 w-5" aria-hidden />}
        title={t("public.publicDrills.soonTitle")}
        hint={t("public.publicDrills.soonHint")}
      />
    </PublicShell>
  );
}
